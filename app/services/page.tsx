  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {serviceCategories.map((s) => (
              <div
                key={s.title}
                className="rounded-3xl border border-gray-100 hover:border-brand-100 bg-gray-50 hover:bg-white p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group"
              >
                <div
                  className="w-14 h-14 rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
                >
      ...
      {false && <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <h2 className="text-2xl md:text-3xl font-medium text-gray-900 mt-2 mb-4">
              Website Packages
            </h2>
            ...
            {websitePlans.map((plan) => (
              <div
                ...
                className={`relative bg-white rounded-3xl shadow-sm border transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl overflow-hidden group ${
                  plan.popular ? 'border-brand-300 shadow-brand-100' : 'border-gray-100'
                }`}
              >
                {plan.popular && (
                  <div className="absolute top-0 left-0 right-0 bg-brand-600 text-white text-xs font-bold text-center py-1.5 tracking-wide">
                    BEST SELLING
                  </div>
                )}

                {/* Header */}
                <div className={`bg-slate-900 p-7 text-white ${plan.popular ? 'pt-9' : ''}`}>
                ...
                <div className="p-7">
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start space-x-2.5 text-sm text-gray-600">
                        <div className={`mt-0.5 w-4 h-4 rounded-full bg-slate-900 flex items-center justify-center shrink-0`}>
                  ...
                  <Link
                    ...
                    className={`flex items-center justify-center space-x-2 w-full py-3.5 rounded-xl font-bold text-sm bg-brand-600 text-white hover:shadow-lg transition-all duration-300 group-hover:scale-105`}
                  >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="text-slate-950">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <span className="text-brand-700 text-sm font-semibold uppercase tracking-widest">Mobile Apps</span>
              </div>
              <h3 className="text-2xl font-medium mb-2">Custom Mobile App</h3>
              <p className="text-slate-600 text-lg">
                iOS & Android apps starting from{' '}
                <span className="font-bold text-slate-950">₹79,999</span>. Pricing
                depends on features — let&apos;s talk!
              </p>
            </div>
            <Link
              href="/contact"
              className="shrink-0 flex items-center space-x-2 border border-[#f58220] bg-white text-gray-900 px-8 py-4 rounded-full font-bold text-sm hover:text-[#f58220] transition-all duration-300"
            >
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <span className="text-brand-600 font-semibold text-xs uppercase tracking-widest">How It Works</span>
            <h2 className="text-2xl font-medium text-gray-900 mt-2 mb-4">Our Process</h2>
            <p className="text-gray-500 max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            {[
              ...
              <div key={i} className="text-center relative">
                <div className="bg-brand-50 border border-brand-100 text-brand-700 w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-bold mx-auto mb-5">
                  {item.step}
                </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {whyUs.map((w) => (
              <div key={w.title} className="text-center p-6 rounded-2xl bg-gray-50 hover:bg-brand-50 transition-colors duration-200 group">
                <div className="w-12 h-12 bg-brand-100 group-hover:bg-brand-200 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-colors duration-200">
                  <w.icon className="h-6 w-6 text-brand-600" />
                </div>
                <h4 className="font-semibold text-gray-900 text-sm mb-2">{w.title}</h4>
